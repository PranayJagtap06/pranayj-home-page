...
const loadConfig = () => {
    if (window.DROPBOX_CLIENT_ID && window.DROPBOX_CLIENT_SECRET) {
        ...
        console.log('Window ClientId:', window.DROPBOX_CLIENT_ID);
        console.log('Final ClientId:', config.DROPBOX_API_CONFIG.clientId);
        console.log('Window ClientSecret:', window.DROPBOX_CLIENT_SECRET ? 'Present' : 'Missing');
        console.groupEnd();

        console.log('Dropbox Config Loading:', {
            windowClientId: window.DROPBOX_CLIENT_ID,
            metaEnvClientId: import.meta.env?.VITE_DROPBOX_CLIENT_ID,
            finalClientId: config.DROPBOX_API_CONFIG.clientId,

            windowClientSecret: window.DROPBOX_CLIENT_SECRET ? 'Present' : 'Missing',
